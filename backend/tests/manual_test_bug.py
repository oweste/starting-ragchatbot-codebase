...
def test_max_results_bug():
    ...
    if results.error:
        ...
        print(
            f"   This causes ChromaDB query to request n_results={vector_store.max_results}"
        )
        print(f"   which returns zero documents.")
    else:
        print(f"\n✅ Search returned {len(results.documents)} results:")
        for i, (doc, meta) in enumerate(
            zip(results.documents[:2], results.metadata[:2])
        ):
            print(f"\n  Result {i+1}:")
    ...
    if config.MAX_RESULTS == 0:
        ...
        print(
            f"   This causes all searches without explicit limits to return 0 results."
        )
        print(
